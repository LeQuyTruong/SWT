...
//Tim code chua active theo ma code va userid
...
//Tim code chua active theo ma code va userid va otpId
    public OTPRequest checkCodeActive(int codeId, int user_id, String code) {
        String sql_account
                = "SELECT [id]\n"
                + "      ,[code]\n"
                + "      ,[type]\n"
                ...
                + "      ,[isDeleted]\n"
                + "  FROM [dbo].[OTPRequest]\n"
                + "  WHERE id = ? AND isVerify = 0 AND createdBy = ? AND isDeleted = 0 and code = ?";
        OTPRequest co = new OTPRequest();
        try {
            PreparedStatement st = connection.prepareStatement(sql_account);
            st.setInt(1, codeId);
