...
public class DebtorDBContext extends DBContext {
    ...
    public boolean save(Debtor debtorDTO) throws SQLException, ClassNotFoundException {
     ...
     public String createNewDebtor(Debtor debtorDTO, int user_id, String txt) {
        HandleCookies hc = new HandleCookies();
        String cookies = "";
        String sql
        ...
        try {
            ...
            st1.executeUpdate();
            cookies = hc.addNoti(user_id, "success", "CreateDebtor", txt);
        } catch (Exception e) {
            e.printStackTrace();
            cookies = hc.addNoti(user_id, "fail", "CreateDebtor", txt);
    ...
    public Debtor getDebtorByID(int id) {
     ...
     public String editDebtor(String name, String address, String phone_number, int id, String txt, int user_id) {
        HandleCookies hc = new HandleCookies();
      
        PreparedStatement stm = null;
        String sql = "  UPDATE [dbo].[Debtor]\n"
                + "   SET [name] = ?\n"
                + "      ,[address] = ?\n"
                + "      ,[phone_number] = ?\n"
                + "      ,[updatedAt] = GETDATE()"
        ...
        try {
            ...
            stm.setInt(4, id);
            stm.executeUpdate();
               String cookies = hc.addNoti(user_id, "success", "EditDebtor", txt);
               return cookies;
        } catch (Exception e) {
            System.out.println(e);
            String cookies   = hc.addNoti(user_id, "fail", "EditDebtor", txt);
             return cookies;
        }
